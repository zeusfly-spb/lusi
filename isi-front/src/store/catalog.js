...
export default {
            ...
            return new Promise((resolve, reject) => {
                Vue.axios.post('/api/set_service_highlight', {...data})
                    .then(res => {
                        ...
                        let text = `Изменен цвет подсветки для улуги "${res.data.description}"`
                        dispatch('pushMessage', {text: text})
                        resolve(res)
                    })
                    .catch(e => reject(e))
            })
    ...
    getters: {
        ...
        colorValue: state => classname => {
            ...
            let level = +classname.split(' ')[1].split('-')[1]
            level === 5 ? level = level * 10 : level = 100 * (5 - level)
            let baseObject = state.colors[baseColor]
            return baseObject && baseObject[level] || ''
        },
